...
-- +---+---+------+
moves as (
     select how_much - 1 as idx, cast(f as int) as f, cast(t as int) as t, row_number() over (order by row_num) as step from moves_prep
),
solution as (
     ...
     select
        head,
        iif(stack_num = moves.f and row_num = json_extract(max_map, printf("$[%s]", stack_num-1)) - idx,
            moves.t,
            stack_num) as stack_num,
        case when stack_num = moves.f and row_num = json_extract(max_map, printf("$[%s]", stack_num-1)) - idx
            then json_extract(max_map, printf("$[%s]", moves.t-1)) + 1
            else case when stack_num = moves.f and row_num > json_extract(max_map, printf("$[%s]", stack_num-1)) - idx
                then row_num - 1
                else row_num
            end
        end as row_num,
